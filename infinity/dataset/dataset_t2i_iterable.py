...
class T2IIterableDataset(IterableDataset):
    ...
    def __iter__(self):
        ...
        for _ in range(self.samples_div_gpus_workers_batchsize_2batches):
            ...
            # images_con: [B, 3, H, W]
            images_con = images.clone()

            # # 转换为灰度图
            # gray = kornia.color.rgb_to_grayscale(images_con)

            # # 应用Canny
            # edges = kornia.filters.canny(
            #     gray,
            #     low_threshold=0.1,
            #     high_threshold=0.2,
            #     kernel_size=(3, 3),
            ...
            # # edges[0]是边缘图 [B, 1, H, W]
            # images_con = edges[0].repeat(1, 3, 1, 1)  # 转回3通道

            images_con = F.interpolate(images_con, size=(256,256), mode='bicubic')
            # images_con = F.interpolate(images_con, size=(1024,1024), mode='bicubic')
            
            yield (images, images_con, captions)
            del batch_data
            del images
            del images_con
            del captions
...
class T2IIterableDataset_deg(IterableDataset):
    def __init__(
        self, 
        meta_folder: str, 
        max_caption_len=512, 
        ...
        for _ in range(self.samples_div_gpus_workers_batchsize_2batches):
            ...
            while len(batch_data) < self.batch_size:
                try:
                    ...
                    kernel1,kernel2,sinc_kernel = self.gen_kernel()
                    batch_kernel.append([kernel1,kernel2,sinc_kernel])
            ...
            kernel1s =torch.stack([item[0] for item in batch_kernel])
            kernel2s =torch.stack([item[1] for item in batch_kernel])
            sinc_kernels =torch.stack([item[2] for item in batch_kernel])


            yield (images, captions, kernel1s, kernel2s, sinc_kernels)
            del batch_data
            del images
            del captions
            del batch_kernel
