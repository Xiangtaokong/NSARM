...
class BitwiseSelfCorrection_forcon(object):
    def __init__(self, vae, args):
        self.noise_apply_layers = 0
        self.noise_apply_requant = 0
        self.noise_apply_strength = 0
    ...
    def flip_requant(self, vae_scale_schedule, inp_B3HW, raw_features, device):
        with torch.amp.autocast('cuda', enabled = False):
            ...
            for si, (pt, ph, pw) in enumerate(vae_scale_schedule):
                ...
                if si != len(vae_scale_schedule)-1:
                    residual = F.interpolate(residual, size=vae_scale_schedule[si], mode=self.vae.quantizer.z_interplote_down).contiguous()
                res_list_1024.append(residual)
                quantized, _, bit_indices, loss = self.vae.quantizer.lfq(residual) # quantized shape: [B, d_vae, 1, h, w], bit_indices shape: [B,1,h,w,d_vae]
                print(f'self.vae: {self.vae}')
                gt_all_bit_indices.append(bit_indices)
                pred_all_bit_indices.append(bit_indices)
                cum_var_input = cum_var_input + F.interpolate(quantized, size=vae_scale_schedule[-1], mode=self.vae.quantizer.z_interplote_up).contiguous()
                if si < len(vae_scale_schedule)-1:
            ...
            gt_ms_idx_Bl = [item for item in gt_all_bit_indices] #只用于测试
        ...
        return x_BLC_wo_prefix, gt_ms_idx_Bl, res_list_1024
...
class BitwiseSelfCorrection_forreplace(object):
    def __init__(self, vae, args):
        self.noise_apply_layers = 0
        self.noise_apply_requant = 0
        self.noise_apply_strength = 0
    ...
    def flip_requant(self, vae_scale_schedule, inp_B3HW, raw_features, device):
        with torch.amp.autocast('cuda', enabled = False):
            ...
            pred_all_bit_indices = []
            x_BLC_wo_prefix = []
            vae_scale_schedule=vae_scale_schedule[0:7]

            for si, (pt, ph, pw) in enumerate(vae_scale_schedule):
                this_scale_input=F.interpolate(codes_out, size=vae_scale_schedule[si], mode=self.vae.quantizer.z_interplote_down).contiguous()
                quantized, _, bit_indices, loss = self.vae.quantizer.lfq(this_scale_input)
                x_BLC_wo_prefix.append(quantized.reshape(*quantized.shape[:2], -1).permute(0,2,1))
...
class BitwiseSelfCorrection_forres(object):
    def __init__(self, vae, args):
        self.noise_apply_layers = 0
        self.noise_apply_requant = 0
        self.noise_apply_strength = 0
    ...
    def flip_requant(self, vae_scale_schedule, inp_B3HW, raw_features, device):
        with torch.amp.autocast('cuda', enabled = False):
            ...
            for si, (pt, ph, pw) in enumerate(vae_scale_schedule):
                ...
                quantized, _, bit_indices, loss = self.vae.quantizer.lfq(residual) # quantized shape: [B, d_vae, 1, h, w], bit_indices shape: [B,1,h,w,d_vae]
                gt_all_bit_indices.append(bit_indices)
                gt_all_o_residual.append(residual)
                gt_all_q_residual.append(quantized)
            ...
            if self.apply_spatial_patchify:
                ...
                for item in gt_all_bit_indices:
                    ...
                    item = item.permute(0,2,3,1).reshape(B, -1, 4*self.vae.codebook_dim)
                    gt_ms_idx_Bl.append(item)
                    q_residual_codes = [item.reshape(B, -1, self.vae.codebook_dim) for item in gt_all_q_residual]
                    o_residual_codes = [item.reshape(B, -1, self.vae.codebook_dim) for item in gt_all_o_residual]
            else:
                gt_ms_idx_Bl = [item.reshape(B, -1, self.vae.codebook_dim) for item in gt_all_bit_indices]
                q_residual_codes = [item.reshape(B, -1, self.vae.codebook_dim) for item in gt_all_q_residual]
                o_residual_codes = [item.reshape(B, -1, self.vae.codebook_dim) for item in gt_all_o_residual]
            
            # gt_ms_idx_Bl = [item for item in gt_all_bit_indices] 
            # q_residual_codes = [item for item in gt_all_q_residual] 
            # o_residual_codes = [item for item in gt_all_o_residual] 
        ...
        return x_BLC_wo_prefix, gt_ms_idx_Bl, q_residual_codes, o_residual_codes
...
class BitwiseSelfCorrection_re(object):
    def __init__(self, vae, args):
        self.noise_apply_layers = args.noise_apply_layers
        self.noise_apply_requant = args.noise_apply_requant
        self.noise_apply_strength = args.noise_apply_strength
        ...
        # self.noise_apply_requant = 0
        # self.noise_apply_strength = 0
        # self.apply_spatial_patchify = 1
        # self.vae = vae
        # self.debug_bsc = 0

    def flip_requant(self, vae_scale_schedule, pre_o_residual_codes_list,inp_B3HW, raw_features,device):
        with torch.amp.autocast('cuda', enabled = False):
            B = raw_features.shape[0]
            if raw_features.dim() == 4:
                codes_out = raw_features.unsqueeze(2)
            ...
            for si, (pt, ph, pw) in enumerate(vae_scale_schedule):
                ...
                if si != len(vae_scale_schedule)-1:
                    ...
                    #if si <= 100:
                        if self.apply_spatial_patchify:
                            residual = pre_o_residual_codes_list[si].reshape(B,14,1,ph,pw).contiguous()
                        else:
                            residual = pre_o_residual_codes_list[si].reshape(B,32,1,ph,pw).contiguous() 
                    else:
                        residual = F.interpolate(residual, size=vae_scale_schedule[si], mode=self.vae.quantizer.z_interplote_down).contiguous()
...
class BitwiseSelfCorrection_re_test(object):
    def __init__(self, vae, args):
...
class BitwiseSelfCorrection_consistency(object):
    def __init__(self, vae, args):
        self.noise_apply_layers = 0
        self.noise_apply_requant = 0
        self.noise_apply_strength = 0
    ...
    def flip_requant(self, vae_scale_schedule,pre_o_residual_codes_list, raw_features, device):
        with torch.amp.autocast('cuda', enabled = False):
            B = raw_features.shape[0]
            if raw_features.dim() == 4:
                codes_out = raw_features.unsqueeze(2)
            ...
            cum_var_input_list = []
            pre_o_sum_list = []
            vae_scale_schedule_=vae_scale_schedule[:7]
            for si, (pt, ph, pw) in enumerate(vae_scale_schedule_):
                residual = codes_out - cum_var_input

                if si != len(vae_scale_schedule)-1:
                    residual = F.interpolate(residual, size=vae_scale_schedule[si], mode=self.vae.quantizer.z_interplote_down).contiguous()
                cum_var_input = cum_var_input + F.interpolate(residual, size=vae_scale_schedule[-1], mode=self.vae.quantizer.z_interplote_up).contiguous()
                cum_var_input_ = F.interpolate(cum_var_input, size=vae_scale_schedule[si], mode=self.vae.quantizer.z_interplote_down).contiguous()
                cum_var_input_list.append(cum_var_input_)
                
                pre_residual = pre_o_residual_codes_list[si].reshape(B, self.vae.codebook_dim,1,ph,pw)
                pre_o_sum = pre_o_sum + F.interpolate(pre_residual, size=vae_scale_schedule[-1], mode=self.vae.quantizer.z_interplote_up).contiguous()
                pre_o_sum_ = F.interpolate(pre_o_sum, size=vae_scale_schedule[si], mode=self.vae.quantizer.z_interplote_down).contiguous()
                pre_o_sum_list.append(pre_o_sum_)
            

            cum_var_input_list_ = [item.reshape(B, -1, self.vae.codebook_dim) for item in cum_var_input_list]
            pre_o_sum_list_ = [item.reshape(B, -1, self.vae.codebook_dim) for item in pre_o_sum_list]

            cum_var_input_list__ = torch.cat(cum_var_input_list_, dim=1).contiguous()
            pre_o_sum_list__ = torch.cat(pre_o_sum_list_, dim=1).contiguous()

        return cum_var_input_list__, pre_o_sum_list__
