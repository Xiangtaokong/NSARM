...
class SelfAttnBlock(nn.Module):
    def __init__(
        ...
        swiglu=False, customized_flash_attn=False, fused_mlp=False, fused_norm_func=None, checkpointing_sa_only=False,use_flex_attn=False,
        batch_size=2, pad_to_multiplier=1, apply_rope2d=False, rope2d_normalized_by_hw=False,
    ):
    ...
    # NOTE: attn_bias_or_two_vector is None during inference
    def forward(self, x, cond_BD, ca_kv=None, attn_bias_or_two_vector=None,attn_fn=None,scale_schedule=None, rope2d_freqs_grid=None):  # todo: minGPT and vqgan also uses pre-norm, just like this, while MaskGiT uses post-norm
        ...
        if self.fused_ada_norm is None:
            ...
            x = x + self.drop_path(self.attn( x_sa,attn_bias_or_two_vector=attn_bias_or_two_vector, attn_fn = attn_fn, scale_schedule=scale_schedule, rope2d_freqs_grid=rope2d_freqs_grid).mul_(gamma1))
