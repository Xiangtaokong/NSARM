...
from infinity.models.bitwise_self_correction import BitwiseSelfCorrection, BitwiseSelfCorrection_forcon
...
class MultipleLayers_con(nn.Module):
    ...
    def forward(self, x, cond_BD, ca_kv, attn_bias_or_two_vector, attn_fn=None, scale_schedule=None, checkpointing_full_block=False, rope2d_freqs_grid=None,si=0):
        h = x
        h_list=[]
        for m in self.module:
            if self.training:
                ...
                h = m(h, cond_BD, ca_kv, attn_bias_or_two_vector, attn_fn, scale_schedule, rope2d_freqs_grid,scale_ind=si)
            h_list.append(h)
        return h, h_list

class MultipleLayers_cat(nn.Module):
    def __init__(self, ls, num_blocks_in_a_chunk, index):
        super().__init__()
        self.module = nn.ModuleList()
        for i in range(index, index+num_blocks_in_a_chunk):
        ...
        car_skip_norm = []
        car_skip_linear = []
        for _ in range(num_blocks_in_a_chunk):
            car_skip_norm.append(FP32_Layernorm(2 * self.C, elementwise_affine=True, eps=1e-6))
    ...
    def forward(self, x, cond_BD, ca_kv, attn_bias_or_two_vector, attn_fn=None, scale_schedule=None, checkpointing_full_block=False, rope2d_freqs_grid=None,con_x=None,si=0):
        control_index = 0
        ...
        for m in self.module:
            ...
            # con_f = con_x[control_index]
            # cat = torch.cat([h, con_f], dim=-1)
            # cat = self.car_skip_norm[control_index](cat)
            # h = self.car_skip_linear[control_index](cat)
            ...
            # # # 保持前521个token变，only for train，前521个token是到256分辨率，推理的时候用scale来控制
            # preserve_tokens = 521
            # h[:,preserve_tokens:,:] = x[:,preserve_tokens:,:]
...
class ImageControlEmbedding(nn.Module):
    def __init__(
        self,
...
class Map_Transformer(nn.Module):
    def __init__(self, mlp_ratio=4.0, dropout=0):
        super().__init__()
        
        # 定义每个块的大小和对应的维度
        self.block_sizes = [1*1, 2*2, 4*4, 6*6, 8*8, 12*12, 16*16]
        
        # 根据块大小调整维度 - 较小的块使用较小的维度
        self.block_dims = [
            64,     # 1x1 块使用较小的维度
            128,    # 2x2 块
            256,    # 4x4 块
        ...
        for size in self.block_sizes:
            running_sum += size
            self.block_indices.append(running_sum)
        
        # 输入维度
        self.input_dim = 32
        
        # 为每个块创建输入投影、Transformer和输出投影
        self.input_projections = nn.ModuleList()
        self.transformers = nn.ModuleList()
        self.output_projections = nn.ModuleList()
        
        for i, (size, dim) in enumerate(zip(self.block_sizes, self.block_dims)):
            # 输入投影
            self.input_projections.append(nn.Linear(self.input_dim, dim))
            ...
            self.transformers.append(nn.TransformerEncoder(encoder_layer, num_layers=layers))
            
            # 输出投影
            self.output_projections.append(nn.Linear(dim, self.input_dim))
    ...
    def forward(self, x):
        ...
        for i in range(len(self.block_sizes)):
            start_idx = self.block_indices[i]
            end_idx = self.block_indices[i+1]
            
            # 提取当前块
            block = x[:, start_idx:end_idx, :]
            
            # 投影到更高维度
            block = self.input_projections[i](block)
            
            # 应用对应的Transformer
            transformed = self.transformers[i](block)
            
            # 投影回原始维度
            transformed = self.output_projections[i](transformed)
...
class Infinity(nn.Module):
    def __init__(
        ...
        self.map_transformer = Map_Transformer()
        # [backbone and head]
        ...
        self.unregistered_con_blocks = []
        for block_idx in range(int(depth /2)):
            block = (CrossAttnBlock if self.t2i else SelfAttnBlock)(
                embed_dim=self.C, kv_dim=self.D, cross_attn_layer_scale=cross_attn_layer_scale, cond_dim=self.D, act=True, shared_aln=shared_aln, norm_layer=norm_layer,
                num_heads=num_heads, mlp_ratio=mlp_ratio, drop=drop_rate, drop_path=dpr[block_idx], tau=tau, cos_attn=cos_attn,
                swiglu=swiglu, customized_flash_attn=self.customized_flash_attn, fused_mlp=fused_mlp, fused_norm_func=fused_norm_func,
            ...
            self.unregistered_con_blocks.append(block)
        ...
        if self.num_block_chunks == 1:
            ...
            for i in range(self.num_block_chunks):
                if i < self.num_block_chunks /2:
                    self.block_chunks.append(MultipleLayers(self.unregistered_blocks, self.num_blocks_in_a_chunk, i*self.num_blocks_in_a_chunk))
                else:
                    self.block_chunks.append(MultipleLayers_cat(self.unregistered_blocks, self.num_blocks_in_a_chunk, i*self.num_blocks_in_a_chunk))
            
            # # 添加用于处理控制特征的Transformer块
            self.car_block_chunks = nn.ModuleList()
            for i in range(int(self.num_block_chunks /2)):
                self.car_block_chunks.append(MultipleLayers_con(self.unregistered_con_blocks, self.num_blocks_in_a_chunk, i*self.num_blocks_in_a_chunk))
        print(
    ...
    def forward(self, label_B_or_BLT: Union[torch.LongTensor, Tuple[torch.FloatTensor, torch.IntTensor, int]], x_BLC_wo_prefix: torch.Tensor, x_BLC_wo_prefix_replace: torch.Tensor, scale_schedule: List[Tuple[int]],
        cfg_infer=False,
        **kwargs,
        ...
        x_BLC_wo_prefix_replace = x_BLC_wo_prefix_replace.float()
        B = x_BLC_wo_prefix.shape[0]

        print(f'x_BLC_wo_prefix: {x_BLC_wo_prefix.shape}',flush=True)
        print(f'x_BLC_wo_prefix_replace: {x_BLC_wo_prefix_replace.shape}',flush=True)

        x_BLC_wo_prefix_traget = x_BLC_wo_prefix[:,0:520]
        # 获取 x_BLC_wo_prefix_replace 第二维的第一个元素
        first_element = x_BLC_wo_prefix_replace[:, 0:1, :]
        # 在 x_BLC_wo_prefix_traget 的第二维前面增加这个元素
        x_BLC_wo_prefix_traget = torch.cat([first_element, x_BLC_wo_prefix_traget], dim=1)

        print(f'x_BLC_wo_prefix_traget: {x_BLC_wo_prefix_traget.shape}', flush=True)
        print(f'x_BLC_wo_prefix_replace: {x_BLC_wo_prefix_replace.shape}', flush=True)

        x_BLC_wo_prefix_pre = self.map_transformer(x_BLC_wo_prefix_replace)

        print(f'x_BLC_wo_prefix_pre: {x_BLC_wo_prefix_pre.shape}', flush=True)
        
        x_BLC_wo_prefix = torch.cat([first_element, x_BLC_wo_prefix], dim=1)
        
        print(f'x_BLC_wo_prefix: {x_BLC_wo_prefix.shape}',flush=True)

        x_BLC_wo_prefix[:,0:521] = x_BLC_wo_prefix_pre
        ...
        with torch.amp.autocast('cuda', enabled=False):
            ...
            x_BLC = self.word_embed(self.norm0_ve(x_BLC_wo_prefix))



            #print(f"x_BLC_replace.shape: {x_BLC.shape}")
            
            #x_BLC_replace = torch.cat((sos, self.word_embed(self.norm0_ve(x_BLC_wo_prefix_replace))), dim=1)
            
            #x_BLC_replace = x_BLC_replace + x_BLC #v2 add
            #v3 no add
            ...
            # print(f"need_to_pad: {need_to_pad}")
            if self.customized_flash_attn:
                ...
                if need_to_pad:
                    ...
                    x_BLC_replace = F.pad(x_BLC_replace, (0, 0, 0, need_to_pad))
        ...
        # print(f"x_BLC.shape: {x_BLC.shape}")
        # print(f"x_BLC_replace.shape: {x_BLC_replace.shape}")

        # print(attn_fn.shape)
        if self.add_lvl_embeding_only_first_block:
            x_BLC = self.add_lvl_embeding_for_x_BLC(x_BLC, scale_schedule, need_to_pad)
            x_BLC_replace = self.add_lvl_embeding_for_x_BLC(x_BLC_replace, scale_schedule, need_to_pad)
        ...
        for i, chunk in enumerate(self.car_block_chunks): # this path
            x_BLC_replace, x_BLC_replace_list = chunk(x=x_BLC_replace, cond_BD=cond_BD_or_gss, ca_kv=ca_kv, attn_bias_or_two_vector=attn_bias_or_two_vector, attn_fn=attn_fn, scale_schedule=scale_schedule, checkpointing_full_block=checkpointing_full_block, rope2d_freqs_grid=self.rope2d_freqs_grid)
            control_residual_f.append(x_BLC_replace_list)
            #x_BLC_replace_list:记录一个chunk中每个layer的输出
        # for cb in self.car_blocks:
        #     x_BLC_replace = cb(x=x_BLC_replace, cond_BD=cond_BD_or_gss, ca_kv=ca_kv, attn_bias_or_two_vector=attn_bias_or_two_vector, attn_fn=attn_fn, scale_schedule=scale_schedule, rope2d_freqs_grid=self.rope2d_freqs_grid)
        #     control_residual_f.append(x_BLC_replace)
        ...
        if self.num_block_chunks == 1:
            ...
            for i, chunk in enumerate(self.block_chunks): # this path
                if i < self.num_block_chunks /2:
                    ...
                    con_f = control_residual_f[control_index] # 取出当前chunk的几个layer的con，送给chunk_act分别处理
                    x_BLC = chunk(x=x_BLC, cond_BD=cond_BD_or_gss, ca_kv=ca_kv, attn_bias_or_two_vector=attn_bias_or_two_vector, attn_fn=attn_fn, scale_schedule=scale_schedule, checkpointing_full_block=checkpointing_full_block, rope2d_freqs_grid=self.rope2d_freqs_grid,con_x=con_f)
                    control_index+=1
        # [3. unpad the seqlen dim, and then get logits]
        print(f'logits_BLV: {self.get_logits(x_BLC[:, :l_end], cond_BD)}',flush=True)
        return self.get_logits(x_BLC[:, :l_end], cond_BD),x_BLC_wo_prefix_traget    # return logits BLV, V is vocab_size
    ...
    def autoregressive_infer_cfg(
        ...
        h_div_w_template = h_div_w_templates[np.argmin(np.abs(1 - h_div_w_templates))]

        # with open("/home/notebook/data/personal/S9053103/Infinity/001_256x4.png", 'rb') as f:
        # with open("/mnt/data/cpfs/cfps/personal/kxt/Infinity/test_img/bird_1024.png", 'rb') as f:
        with open("/mnt/data/cpfs/cfps/personal/kxt/Infinity/test_img/bird_256x4.png", 'rb') as f:
        # with open("/mnt/data/cpfs/cfps/personal/kxt/Infinity/test_img/000060_256x4.png", 'rb') as f:
        # with open("/mnt/data/cpfs/cfps/personal/kxt/Infinity/test_img/000060.png", 'rb') as f:
            
            img: PImage.Image = PImage.open(f)
            img = img.convert('RGB')

            tgt_h, tgt_w = dynamic_resolution_h_w[h_div_w_template]["1M"]['pixel']
            img_B3HW = transform(img, tgt_h, tgt_w).unsqueeze(0).unsqueeze(2)

        
        img_B3HW=img_B3HW.to(last_stage.device, non_blocking=True)
        
        raw_features, _, _ = vae.encode_for_raw_features(img_B3HW, scale_schedule=scale_schedule)

        self.bitwise_self_correction=BitwiseSelfCorrection_forcon(vae,None)

        x_BLC_wo_prefix, gt_ms_idx_Bl = self.bitwise_self_correction.flip_requant(scale_schedule, img_B3HW, raw_features, last_stage.device)
        

        x_BLC_replace = x_BLC_wo_prefix


        gt_codes_1 = vae.quantizer.lfq.indices_to_codes(gt_ms_idx_Bl[-1], label_type='bit_label')

        gt_ls_Bl = gt_ms_idx_Bl
        #gt_leak = 6

        print(gt_ls_Bl[12].shape)
 
        ###############################################################################################
        
        
        control_f = x_BLC_replace


        print("control_f")
        print(control_f.shape)
        ...
        for si, pn in enumerate(scale_schedule):   # si: i-th segment
            ...
            print(f"self.use_flex_attn: {self.use_flex_attn}")
            

            control_x = control_f[:,pn_start:pn_start+pn[1]*pn[2],:]
            #control_x = self.word_embed(self.norm0_ve(control_x)).repeat(2, 1, 1)
            control_x = self.word_embed(self.norm0_ve(control_x))

            pn_start += pn[1]*pn[2]

            #control_x = last_stage + control_x
            # 第一个循环 control_x 是 图像条件，last_stage 是 单纯的sos
            ...
            # control_residual_f = []
            # for cb in self.car_blocks:
            #     control_x = cb(x=control_x, cond_BD=cond_BD_or_gss)
            #     control_residual_f.append(control_x)


            # control_residual_f = []  #这是在一个迭代中，生成一个scale，需要对网络的每一层都控制
            # for i, chunk in enumerate(self.car_block_chunks): # this path
            #     m_list = []
            #     for m in chunk.module:
            #         control_x = m(control_x, cond_BD=cond_BD_or_gss, ca_kv=ca_kv, attn_bias_or_two_vector=None, attn_fn=attn_fn, scale_schedule=scale_schedule, rope2d_freqs_grid=self.rope2d_freqs_grid, scale_ind=si)
            #         m_list.append(control_x)
            #     control_residual_f.append(m_list)
            #     #以chunk为组，收集控制条件
            # print("control_residual_f")
            # print(len(control_residual_f))
            # print(len(control_residual_f[1]))

            control_residual_f = []
            for i, chunk in enumerate(self.car_block_chunks): # this path
                control_x, control_x_l = chunk(x=control_x, cond_BD=cond_BD_or_gss, ca_kv=ca_kv, attn_bias_or_two_vector=None, attn_fn=attn_fn, scale_schedule=scale_schedule, checkpointing_full_block=None, rope2d_freqs_grid=self.rope2d_freqs_grid,si=si)
                control_residual_f.append(control_x_l)
            ...
            for i, chunk in enumerate(self.block_chunks): # this path
                if i < self.num_block_chunks /2 :
                    last_stage = chunk(x=last_stage, cond_BD=cond_BD_or_gss, ca_kv=ca_kv, attn_bias_or_two_vector=None, attn_fn=attn_fn, scale_schedule=scale_schedule, checkpointing_full_block=None, rope2d_freqs_grid=self.rope2d_freqs_grid,si=si)
                else:
                    con_f = control_residual_f[control_index] # 取出当前chunk的几个layer的con，送给chunk_act分别处理
                    last_stage = chunk(x=last_stage, cond_BD=cond_BD_or_gss, ca_kv=ca_kv, attn_bias_or_two_vector=None, attn_fn=attn_fn, scale_schedule=scale_schedule, checkpointing_full_block=None, rope2d_freqs_grid=self.rope2d_freqs_grid,con_x=con_f,si=si)
                    control_index+=1
            ...
            # control_index = 0 #取chunk单位的控制条件
            # layer_idx = 0 #控制cfg，数所有层
            # for block_idx, b in enumerate(self.block_chunks):
            #     if block_idx < self.num_block_chunks /2:
            #         print("NO controling")
            #         print(block_idx,self.num_block_chunks /2)
            #         for m in b.module:
            #             last_stage = m(x=last_stage, cond_BD=cond_BD_or_gss, ca_kv=ca_kv, attn_bias_or_two_vector=None, attn_fn=attn_fn, scale_schedule=scale_schedule, rope2d_freqs_grid=self.rope2d_freqs_grid, scale_ind=si)
            #             if (cfg != 1) and (layer_idx in abs_cfg_insertion_layers):
            #                 # print(f'add cfg={cfg} on {layer_idx}-th layer output')
            ...
            #         print("controling")
            #         print(control_index)
            #         con_f = control_residual_f[control_index]
            #         con_f_i=0 #取chunk中第i个layer的控制条件
            #         for m in b.module:
            #             print(con_f_i)
            #             con_f_l = con_f[con_f_i]
            #             cat = torch.cat([last_stage, con_f_l], dim=-1)
            #             cat = b.car_skip_norm[con_f_i](cat)
            #             last_stage = b.car_skip_linear[con_f_i](cat)
            #             last_stage = m(last_stage, cond_BD=cond_BD_or_gss, ca_kv=ca_kv, attn_bias_or_two_vector=None, attn_fn=attn_fn, scale_schedule=scale_schedule, rope2d_freqs_grid=self.rope2d_freqs_grid, scale_ind=si)
            #             if (cfg != 1) and (layer_idx in abs_cfg_insertion_layers):
            #                 # print(f'add cfg={cfg} on {layer_idx}-th layer output')
            #                 last_stage = cfg * last_stage[:B] + (1-cfg) * last_stage[B:]
            #                 last_stage = torch.cat((last_stage, last_stage), 0)   
            ...
            if self.use_bit_label:
                ...
                # if si == 12:
                #     # 64,64,32
                reshape_gt_ls_Bl = gt_ls_Bl[si].reshape(tmp_bs, tmp_seq_len, -1)
                bitwise_acc = (idx_Bld == reshape_gt_ls_Bl).float()
                res_bit_acc = bitwise_acc.mean(-1).mean(0)
                acc_bit_mean=res_bit_acc.mean().item() * 100.
                print(f'acc_bit_mean: {acc_bit_mean}')
