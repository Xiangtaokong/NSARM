...
#                 use_flex_attn=use_flex_attn, batch_size=batch_size, pad_to_multiplier=pad_to_multiplier, rope2d_normalized_by_hw=rope2d_normalized_by_hw,
#             )
#             for block_idx in range(self.num_block_chunks)
#         ])

#         # 添加特征融合相关组件
#         car_skip_norm = []
#         car_skip_linear = []
#         for _ in range(self.num_block_chunks):
...
#     def forward(self, label_B_or_BLT: Union[torch.LongTensor, Tuple[torch.FloatTensor, torch.IntTensor, int]], x_BLC_wo_prefix: torch.Tensor, x_BLC_wo_prefix_con: torch.Tensor, scale_schedule: List[Tuple[int]],
#         cfg_infer=False,
#         **kwargs,
...
#         x_BLC_wo_prefix_con = x_BLC_wo_prefix_con.float()
...
#             print(sos.shape)
#             print(self.word_embed(self.norm0_ve(x_BLC_wo_prefix)).shape)

#             x_BLC_con = self.word_embed(self.norm0_ve(x_BLC_wo_prefix_con))
            
#             #x_BLC_con = torch.cat((sos, self.word_embed(self.norm0_ve(x_BLC_wo_prefix_con))), dim=1)
#             x_BLC_con = x_BLC_con + x_BLC
...
#                     x_BLC_con = F.pad(x_BLC_con, (0, 0, 0, need_to_pad))
...
#             x_BLC_con = self.add_lvl_embeding_for_x_BLC(x_BLC_con, scale_schedule, need_to_pad)


#         control_residual_f = []
#         for cb in self.car_blocks:
#             x_BLC_con = cb(x=x_BLC_con, cond_BD=cond_BD_or_gss, ca_kv=ca_kv, attn_bias_or_two_vector=attn_bias_or_two_vector, attn_fn=attn_fn, scale_schedule=scale_schedule, rope2d_freqs_grid=self.rope2d_freqs_grid)
#             control_residual_f.append(x_BLC_con)
...
#                 cat = self.car_skip_norm[i - self.num_block_chunks](cat)
#                 x_BLC = self.car_skip_linear[i - self.num_block_chunks](cat)

#                 for m in chunk.module.module:
#                     x_BLC = m(x=x_BLC, cond_BD=cond_BD_or_gss, ca_kv=ca_kv, attn_bias_or_two_vector=attn_bias_or_two_vector, attn_fn=attn_fn, scale_schedule=scale_schedule,rope2d_freqs_grid=self.rope2d_freqs_grid)
...
#     @torch.no_grad()
#     def autoregressive_infer_cfg_no_control(
#         self,
#         vae=None,
#         scale_schedule=None,
#         label_B_or_BLT=None,
...
#         control_f = x_BLC_con
#         x_BLC_con_first = sos.unsqueeze(1).expand(bs, 1, -1) + self.pos_start.expand(bs, 1, -1)
...
#             if si == 0:
#                 control_x = x_BLC_con_first
#             else:
#                 control_x = control_f[:,pn_start-1:pn_start+pn[1]*pn[2]-1,:]
#                 control_x = self.word_embed(self.norm0_ve(control_x)).repeat(2, 1, 1)
#                 print(pn_start-1,pn_start+pn[1]*pn[2]-1)
#                 print("control_x")
...
#                 cat = self.car_skip_norm[block_idx - len(self.block_chunks)](cat)
#                 last_stage = self.car_skip_linear[block_idx - len(self.block_chunks)](cat)
...
class Infinity(nn.Module):
    def __init__(
        ...
        # 添加用于处理控制特征的Transformer块
        if self.num_block_chunks == 1:
            self.car_blocks = nn.ModuleList(self.unregistered_blocks)
        else:
            self.car_block_chunks = nn.ModuleList()
            for i in range(int(self.num_block_chunks /2)):
                self.car_block_chunks.append(MultipleLayers(self.unregistered_blocks, self.num_blocks_in_a_chunk, i*self.num_blocks_in_a_chunk))


        # 添加特征融合相关组件
        car_skip_norm = []
        car_skip_linear = []
        for _ in range(int((self.num_block_chunks*self.num_blocks_in_a_chunk) /2)):
            car_skip_norm.append(FP32_Layernorm(2 * self.C, elementwise_affine=True, eps=1e-6))
            car_skip_linear.append(nn.Linear(2 * self.C, self.C))
        self.car_skip_norm = nn.ModuleList(car_skip_norm)
        self.car_skip_linear = nn.ModuleList(car_skip_linear)
    ...
    def forward(self, label_B_or_BLT: Union[torch.LongTensor, Tuple[torch.FloatTensor, torch.IntTensor, int]], x_BLC_wo_prefix: torch.Tensor, x_BLC_wo_prefix_con: torch.Tensor, scale_schedule: List[Tuple[int]],
        ...
        **kwargs,
        ...
        for i, chunk in enumerate(self.car_block_chunks): # this path
            x_BLC_con = chunk(x=x_BLC_con, cond_BD=cond_BD_or_gss, ca_kv=ca_kv, attn_bias_or_two_vector=attn_bias_or_two_vector, attn_fn=attn_fn, scale_schedule=scale_schedule, checkpointing_full_block=checkpointing_full_block, rope2d_freqs_grid=self.rope2d_freqs_grid)
            control_residual_f.append(x_BLC_con)
            # for m in chunk.module.module:
            #     print("training")
            #     x_BLC_con = m(x=x_BLC_con, cond_BD=cond_BD_or_gss, ca_kv=ca_kv, attn_bias_or_two_vector=attn_bias_or_two_vector, attn_fn=attn_fn, scale_schedule=scale_schedule,rope2d_freqs_grid=self.rope2d_freqs_grid)
            #     control_residual_f.append(x_BLC_con)
        ...
        if self.num_block_chunks == 1:
            ...
            control_idx = 0
            for i, chunk in enumerate(self.block_chunks): # this path
                if i < int(self.num_block_chunks /2):
                    x_BLC = chunk(x=x_BLC, cond_BD=cond_BD_or_gss, ca_kv=ca_kv, attn_bias_or_two_vector=attn_bias_or_two_vector, attn_fn=attn_fn, scale_schedule=scale_schedule, checkpointing_full_block=checkpointing_full_block, rope2d_freqs_grid=self.rope2d_freqs_grid)
                else:
                    con_f = control_residual_f[control_idx]
                    cat = torch.cat([x_BLC, con_f], dim=-1)
                    cat = self.car_skip_norm[control_idx](cat)
                    x_BLC = self.car_skip_linear[control_idx](cat)
                    x_BLC = chunk(x=x_BLC, cond_BD=cond_BD_or_gss, ca_kv=ca_kv, attn_bias_or_two_vector=attn_bias_or_two_vector, attn_fn=attn_fn, scale_schedule=scale_schedule, checkpointing_full_block=checkpointing_full_block, rope2d_freqs_grid=self.rope2d_freqs_grid)
                    control_idx += 1
                    print(control_idx)

                    # for m in chunk.module.module:
                    #     con_f = control_residual_f[layer_idx]
                    #     cat = torch.cat([x_BLC, con_f], dim=-1)
                    #     cat = self.car_skip_norm[layer_idx](cat)
                    #     x_BLC = self.car_skip_linear[layer_idx](cat)
                    #     x_BLC = m(x=x_BLC, cond_BD=cond_BD_or_gss, ca_kv=ca_kv, attn_bias_or_two_vector=attn_bias_or_two_vector, attn_fn=attn_fn, scale_schedule=scale_schedule,rope2d_freqs_grid=self.rope2d_freqs_grid)
                    #     layer_idx += 1
