...
from infinity.utils.color_adjust import color_adjust
...
def gen_one_img(
    ...
    with torch.cuda.amp.autocast(enabled=True, dtype=torch.bfloat16, cache_enabled=True):
        _, _, img_list = infinity_test.autoregressive_infer_cfg(
            ...
            ret_img=True, trunk_scale=trunk_scale,
...
def load_infinity(
    ...
    with torch.cuda.amp.autocast(enabled=True, dtype=torch.bfloat16, cache_enabled=True), torch.no_grad():
        ...
        #     from transformers.modeling_utils import load_sharded_checkpoint
        #     load_sharded_checkpoint(infinity_test, model_path, strict=False)
        #     output_dir = "/home/notebook/data/group/kxt/Infinity/weights/INF_8B_fromtest"
        #     output_path = "/home/notebook/data/group/kxt/Infinity/weights/INF_8B_fromtest.pth"
        #     torch.save(infinity_test.state_dict(), output_path)
        ...
        if args.enable_model_cache:
            ...
            if not osp.exists(local_slim_model_path):
                if osp.exists(slim_model_path):
                    ...
                    save_slim_model(local_model_path, save_file=local_slim_model_path, device='cpu')
...
def add_common_arguments(parser):
    ...
    parser.add_argument('--adain', type=int, default=1, choices=[0,1])
    parser.add_argument('--wavelet', type=int, default=1, choices=[0,1])
...
if __name__ == '__main__':
    ...
    def crop_image_from_1024_cv2(padded_image):
        """
        从1024x1024的OpenCV图像中裁掉黑边，恢复为512x512
        
        Args:
            padded_image: OpenCV图像 (numpy array)，尺寸为(1024, 1024, 3)或(1024, 1024)
            
        Returns:
            OpenCV图像 (numpy array)，尺寸为(512, 512, 3)或(512, 512)
        """
        print(padded_image.shape)
        if padded_image.shape[:2] != (1024, 1024):
            raise ValueError("输入图像必须是1024x1024像素")
        ...
        # 裁剪图像
        cropped_image = padded_image[y_offset:y_offset+512, x_offset:x_offset+512]
    ...
    supir_total, supir_trainable = count_parameters(infinity)
    print(f"\n[INF Model Parameters]")
    print(f"Total parameters: {supir_total:,}")
    print(f"Trainable parameters: {supir_trainable:,}")
    print(f"Non-trainable parameters: {supir_total - supir_trainable:,}")
    ...
    with autocast(dtype=torch.bfloat16):
        with torch.no_grad():
            for info_file in sorted(os.listdir(args.input_info)):
                file_path = os.path.join(args.input_info,info_file)
                save_dir_ = os.path.join(save_dir,info_file[:-6])
                with open(file_path, 'r', encoding='utf-8') as f:
                    for line in f:
                        img_path=json.loads(line)['image_path']
                        with open(img_path, 'rb') as f:
                            print(img_path)
                            img: PImage.Image = PImage.open(f)
                            LR_img = img.convert('RGB')
                        img_prompt=json.loads(line)['long_caption']
                        #img_prompt = 'A high-definition image with harmonious colors and rich details.'
                        basenema=os.path.basename(img_path)

                        SR_image = gen_one_img(
                            infinity,
                            vae,
                            text_tokenizer,
                            text_encoder,
                            ...
                            gt_ls_Bl=None,
                            negative_prompt= "",
                            # "Blurry images, discordant words, and the words are in a complete mess.Unreasonable details, unreasonable numbers, unreasonable letters."
                            cfg_list=args.cfg,
                            tau_list=args.tau,
                            scale_schedule=scale_schedule,
                        ...
                        save_path=os.path.join(save_dir_,f'{basenema}')


                        SR_image=Image.fromarray(SR_image.cpu().numpy())
                        SR_image=color_adjust(SR_image,LR_img,args.adain,args.wavelet)
